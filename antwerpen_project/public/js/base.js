(function () {
    ...
    app.controller("UsersController", ['$http', '$scope', function ($http, $scope) {
        ...
        $http({
            ...
            $scope.error_users_management = "Fout met server, vernieuw je venster aub";
        });
                    ...
                    $http.post(root + "/locatie/toevoegen/" + project_id + "/api", data).success(function (data) {
                        ...
                        if (data.$succes) {
                           // console.log("--edit-- --api call-- - function- -succes-", data.$location);
                            ...
                            $scope.locations_errors = "Oeps, er ging iets verkeerd!";
...
console.log("--home page-- --aswers feadback-- server answer:0",response.data);
                        if (response.data.succes) {
                            ...
                            $scope.answer_post_error= "Oeps, er ging iets fout. Kunt u de pagina vernieuwen en het nog eens proberen aub. "
                        }
                ...
                $scope.submit_reaction = function (user_id) {
                    if ($scope.selected_rating) {
                        if (!$scope.reaction_post.massage) {
                            $scope.project.post_error = "Invoerveld is verplicht."
                        }
                        $http({
                            method: 'POST',
                        ...
                        $scope.project.post_error = "Met hoeveel sterren waardeert u dit project?"
                    }
